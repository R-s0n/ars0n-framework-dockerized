...
function App() {
	...
	useEffect(() => {
		...
		axios
			.post("http://localhost:8000/api/fqdn/all", {})
			.then((res) => {
	...
	const deleteFqdn = () => {
		...
		axios
			.post("http://localhost:8000/api/fqdn/delete", fqdnToDelete)
			.then((res) => {
	...
	const runWildfire = () => {
		...
		if (!scanSingleDomain) {
			...
			axios
				.post("http://localhost:5000/wildfire", payload)
				.then((res) => {
					setScanRunning(true);
	...
	const handleCollectScreenshotsButton = () => {
		axios.post("http://localhost:5000/collect_sceenshots", {}).then((res) => {
	...
	return (
		<div>
			...
			<nav
				className="p-1 pt-2 pb-2 navbar navbar-expand-lg bg-dark"
					...
					<div className="collapse navbar-collapse" id="navbarSupportedContent">
						<ul className="navbar-nav me-auto mb-2 mb-lg-0">
							<li className="nav-item">
								...
								<button className="btn btn-success mr-2" onClick={addNewFqdn}>
									Add FQDN
								</button>
								<button className="btn btn-danger" onClick={deleteFqdn}>
									Delete FQDN
								</button>
								<div
									style={{
							...
							<li className="nav-item ml-5">
								...
								<div className="form-group">
									...
									<button
										className="border border-info btn btn-primary text-secondary ml-2"
										onClick={exportData}
									>
										Export Data
									</button>
							...
							<li>
								<button
									className="border border-info btn btn-primary text-secondary m-4 p-3 ml-5"
									onClick={handleCollectScreenshotsButton}
								>
									Collect Screenshots
								</button>
							</li>
			...
			<div
				className="pl-3 p-2 navbar navbar-expand-lg bg-dark"
				style={{ overflow: "auto", whiteSpace: "nowrap" }}
			>
				<span
					style={{
				...
				>
					Scan Step: {scanStep} / {scanComplete}
					<br></br>
				...
				>
					...
					<option value="slowburn" disabled>
						Slowburn.py
					</option>
					<option value="scorched-earth" disabled>
						ScorchedEarth.py
					</option>
				</select>
				<label style={{ padding: "15px", color: "#D9D9D9" }} for="checkbox1">
					Fire-Starter
				</label>
				<input
					style={{ padding: "15px" }}
					...
					checked={fireStarter}
				/>
				<label style={{ padding: "15px", color: "#D9D9D9" }} for="checkbox2">
					Fire-Cloud
				</label>
				<input
					style={{ padding: "15px" }}
					...
					checked={fireCloud}
				/>
				<label style={{ padding: "15px", color: "#D9D9D9" }} for="checkbox3">
					Fire-Scanner
				</label>
				<input
					style={{ padding: "15px" }}
				...
				<input
					...
					name="firecloud"
					className="checkbox"
					onChange={handleEnumToggle}
					checked={fireEnumeration}
					disabled
				/>
				{scanRunning ? (
					<button
						style={{ width: "100px", marginLeft: "15px" }}
						className="border border-info nav-link btn btn-primary text-secondary"
						type="submit"
						disabled
					>
						Cancel
			...
			{noFqdns === false && fqdns.length > 0 && loaded && (
				<Fqdn
					index={activeTab}
					thisFqdn={fqdns[activeTab]}
					buttonFunction={deleteFqdn}
